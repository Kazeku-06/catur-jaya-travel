...
const TripCard = ({
    ...
    const canBook = is_available_for_booking !== undefined ? is_available_for_booking : isAvailable;

    return (
        <motion.div className={
                `bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 ${className}`
            }
            whileHover={
                {y: -5}
            }
            ...
            <Link to={
                ...
                <div className="relative h-48 overflow-hidden">
                    <img src={
                            getImageUrl(trip.image_url || trip.image)
                        }
                        alt={displayName}
                    ...
                    <div className="absolute top-3 left-3">
                        ...
                        is_quota_full ? (
                            ...
                            <Badge variant={
                                canBook ? 'success' : 'error'
                            }>
                                {
                                canBook ? 'Tersedia' : 'Tidak Tersedia'
                            } </Badge>
                        )
                    } </div>
                    ...
                    <div className="space-y-2">
                        ...
                        quota !== undefined && remaining_quota !== undefined && (
                            <div className="text-center text-sm space-y-1">
                                ...
                                <div className={
                                    `font-medium ${
                                        is_quota_full ? 'text-red-600' : remaining_quota <= 2 ? 'text-orange-600' : 'text-green-600'
                                    }`
                                }>
                                    Sisa kuota: {remaining_quota}
                                    dari {quota}
                                    trip
                                </div>
                            </div>

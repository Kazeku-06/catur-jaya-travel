...
const Footer = () => {
  ...
  const socialLinks = [
    ...
    {
      name: 'Instagram',
      // Link Instagram lu udah masuk di sini bro
      href: 'https://www.instagram.com/caturjayamandiritour?igsh=NWxtam9sdml6Z2dz',
      icon: (
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          ...
          >
            <div className="flex items-center space-x-3 text-white">
              <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-md overflow-hidden p-1.5 border border-gray-700">
                <img 
              ...
              <span className="text-xl font-sans tracking-tight uppercase font-bold">Catur Jaya Mandiri</span>
            </div>
            
            <p className="text-[15px] max-w-sm sm:max-w-none text-gray-400">
              Menghadirkan pengalaman berwisata yang menyenangkan, murah tapi tidak murahan di wilayah Malang Raya & Bromo.
            </p>
            ...
            <div className="flex space-x-3 justify-center sm:justify-start">
              {socialLinks.map((social) => (
                <a 
                  key={social.name} 
                  href={social.href} 
                  target="_blank" // FIX: Membuka di tab baru
                  rel="noopener noreferrer" // FIX: Keamanan untuk link eksternal
                  className="p-2.5 bg-gray-800 rounded-lg hover:bg-blue-600 hover:text-white transition-all duration-300 shadow-sm"
                >
                  {social.icon}
                </a>
              ))}
          ...
          <div className="grid grid-cols-2 sm:contents gap-8">
            {Object.entries(footerLinks).map(([key, section], index) => (
              <motion.div
                ...
                viewport={{ once: true }}
              >
                <h3 className="text-white font-semibold text-base mb-5 lg:mb-6">{section.title}</h3>
                  ...
                  {section.links.map((link) => (
                    <li key={link.name}>
                      {link.external ? (
                        ...
                        <Link to={link.href} className="hover:text-blue-400 transition-colors block py-0.5">
                          {link.name}
                        </Link>
                      )}
                    </li>
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-4 text-[15px]">
              ...
              <div className="flex items-center space-x-3">
                ...
                <span className="text-gray-400 font-medium">+62 813-4647-4165</span>
              </div>
            </div>
          </motion.div>

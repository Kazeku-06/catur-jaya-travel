        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
          ...
          >
            <div className="flex items-center space-x-3 text-white">
              ...
              <span className="text-xl font-bold tracking-tight uppercase">Catur Jaya</span>
            </div>
            
            <p className="text-[15px] justify-center max-w-sm sm:max-w-none">
            ...
            <div className="flex space-x-3 justify-center sm:justify-start">
              {socialLinks.map((social) => (
                <a 
                  key={social.name} 
                  href={social.href} 
                  className="p-2.5 bg-gray-800 rounded-lg hover:bg-primary-600 hover:text-white transition-all duration-300 shadow-sm"
                >
                  {social.icon}
                </a>
              ))}
          ...
          {/* Kolom 2 & 3: Links (Diubah jadi 2 kolom di mobile/tablet agar hemat ruang) */}
              ...
              >
                ...
                <ul className="space-y-3.5 text-[15px]">
                  {section.links.map((link) => (
                    <li key={link.name}>
                      {link.external ? (
                        <a href={link.href} target="_blank" rel="noopener noreferrer" className="hover:text-primary-400 transition-colors block py-0.5">
                          {link.name}
                        </a>
                      ) : (
                        <Link to={link.href} className="hover:text-primary-400 transition-colors block py-0.5">
                          {link.name}
                        </Link>
                      )}
                    </li>

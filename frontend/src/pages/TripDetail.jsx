...
const TripDetail = () => {
    ...
    if (loading) return (
        <Layout>
            <div className="max-w-6xl mx-auto animate-pulse p-4"> {/* Skeleton disamakan lebarnya */}
            ...
            <div className="bg-[#F8F9FA] min-h-screen">
                        ...
                        {breadcrumbItems.map((item, index) => (
                            <div key={index} className="flex items-center">
                                ...
                                {item.active ? (
                                    ...
                                    <Link to={item.path} className="hover:text-blue-500 transition-colors">{item.label}</Link>
                                )}
                            </div>
                        ))}
                    </nav>
                ...
                <div className="container mx-auto px-0 sm:px-4 mt-4">
                    <div className="relative h-[250px] md:h-[400px] w-full overflow-hidden sm:rounded-[2rem] shadow-lg">
                        <img src={getImageUrl(images[selectedImage])} alt={trip?.name} className="w-full h-full object-cover object-center" />
                        <div className="absolute inset-0 bg-black/20" />
                    </div>
                </div>

                {/* CONTENT CONTAINER UTAMA - Sekarang max-w-6xl (Sama dengan Travel Detail) */}
                    ...
                    <div className="max-w-5xl mx-auto">
                        ...
                        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white rounded-3xl p-6 shadow-xl mb-6 border border-gray-100">
                            <div className="flex items-center mb-4">
                                ...
                                <div>
                                    <h2 className="text-xl md:text-2xl font-black text-gray-900 leading-tight">{trip?.name}</h2>
                                    <p className="text-gray-400 font-medium text-sm">{trip?.location} • {trip?.duration}</p>
                                </div>
                            </div>
                            <hr className="my-4 border-gray-50" />
                            <div className="flex items-center text-gray-500">
                                <div className="mr-3 text-[#1a66ff] flex-shrink-0">
                                    <svg
                                    ...
                                    >
                                        <path
                                            ...
                                            strokeWidth={1.2} /* Garis tipis bikin kesan mewah & minimalis */
                                ...
                                <span className="text-xs sm:text-sm font-medium">
                                    Mulai dari <span className="text-blue-600 font-black text-base md:text-lg ml-1">{formatCurrency(trip?.price)} / trip</span>
                                </span>
                            </div>
                        </motion.div>
                        ...
                        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="bg-white rounded-3xl p-6 sm:p-8 shadow-lg mb-8 border border-gray-100">
                            ...
                            <p className="text-gray-500 text-sm sm:text-base leading-relaxed mb-8 whitespace-pre-line">{trip?.description}</p>
                            ...
                            {trip?.facilities?.length > 0 ? (
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
                                    {trip.facilities.map((item, index) => (
                                        <div key={index} className="flex items-center p-3 bg-gray-50 rounded-2xl border border-gray-100 hover:border-blue-200 transition-colors">
                                            <div className="flex-shrink-0 bg-blue-100 text-blue-600 p-1.5 rounded-lg mr-3">
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                                </svg>
                                            </div>
                                            <span className="font-bold text-gray-700 text-xs sm:text-sm truncate" title={item}>{item}</span>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                                    ...
                                    <div className="space-y-4 mb-8 px-2">
                                        {trip.itinerary.map((step, index) => (
                                            <div key={index} className="flex space-x-4">
                                                ...
                                                <div className="pb-4">
                                                    <p className="text-sm text-gray-700 font-bold leading-snug">{step}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>

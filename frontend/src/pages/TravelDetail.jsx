            ...
            <div className="bg-[#F8F9FA] min-h-screen">
                    ...
                    <nav className="flex text-sm text-gray-500 space-x-2 overflow-x-auto whitespace-nowrap py-1">
                        {breadcrumbItems.map((item, index) => (
                            <div key={index} className="flex items-center">
                                {index > 0 && <span className="mx-2 text-gray-300 font-light">/</span>}
                                {index === breadcrumbItems.length - 1 ? (
                                    <span className="text-blue-500 font-bold truncate max-w-[150px]">{item.label}</span>
                                ) : (
                                    <Link to={item.href} className="hover:text-blue-500 transition-colors">{item.label}</Link>
                                )}
                            </div>
                        ))}
                    </nav>
                ...
                <div className="container mx-auto px-0 sm:px-4 mt-4">
                    <div className="relative h-[250px] md:h-[400px] w-full overflow-hidden sm:rounded-[2rem] shadow-lg">
                        <img src={getImageUrl(images[selectedImage])} alt={travel?.name} className="w-full h-full object-cover object-center" />
                        <div className="absolute inset-0 bg-black/10" />
                    </div>
                </div>

                {/* MAIN CONTENT - Diubah ke max-w-6xl agar memanjang tapi tidak berlebihan */}
                <div className="container mx-auto px-4 -mt-16 relative z-10 pb-24">
                    <div className="max-w-5xl mx-auto">

                        {/* CARD 1: Judul & Rute */}
                        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white rounded-3xl p-6 shadow-xl mb-6 border border-gray-100">
                            ...
                            <div className="relative space-y-6 pl-6 mb-6">
                                ...
                                <div className="flex items-start gap-4 relative">
                                    <div className="w-2.5 h-2.5 bg-green-500 rounded-full mt-1.5 z-10 ring-4 ring-white"></div>
                                    <div>
                                        <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Penjemputan</p>
                                        <p className="text-base font-extrabold text-gray-800 uppercase">{travel?.origin || 'Kota Asal'}</p>
                                    </div>
                                </div>
                                <div className="flex items-start gap-4 relative">
                                    <div className="w-2.5 h-2.5 bg-red-500 rounded-full mt-1.5 z-10 ring-4 ring-white"></div>
                                    <div>
                                        <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Tujuan</p>
                                        <p className="text-base font-extrabold text-gray-800 uppercase">{travel?.destination || 'Kota Tujuan'}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex items-center text-gray-500">
                                ...
                                <span className="text-xs sm:text-sm font-medium">
                                    Mulai dari <span className="text-blue-600 font-black text-base md:text-lg ml-1">{formatCurrency(travel?.price)} / orang</span>
                                </span>
                            </div>
                        </motion.div>

                        {/* CARD 2: Deskripsi & Fasilitas */}
                        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="bg-white rounded-3xl p-6 sm:p-8 shadow-lg mb-8 border border-gray-100">
                            ...
                            {travel?.facilities?.length > 0 ? (
                                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                                    {travel.facilities.map((facility, i) => (
                                        <div key={i} className="flex items-center p-3 bg-gray-50 rounded-2xl border border-gray-100 group transition-colors">
                                            <div className="flex-shrink-0 text-blue-500 mr-2.5">
                                                <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
                                                </svg>
                                            </div>
                                            <span className="font-bold text-gray-700 text-xs sm:text-sm truncate" title={facility}>{facility}</span>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                        ...
                        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="bg-white rounded-3xl p-6 sm:p-8 shadow-lg mb-8 border border-gray-100">
                                ...
                                {travel?.rundown?.length > 0 ? (travel.rundown.map((item, i) => (
                                    <div key={i} className="flex space-x-4 group">
                                        <div className="flex flex-col items-center">
                                            ...
                                            {i !== travel.rundown.length - 1 && <div className="w-[1px] h-full bg-gray-100 my-1"></div>}
                                        </div>
                                        <div className="pb-6 flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="text-sm font-black text-blue-600">{item.time || item.jam}</span>
                                            </div>
                            ...
                            <p className="text-[10px] text-gray-400 italic mt-2 font-medium border-t pt-4">* Jadwal dapat berubah sewaktu-waktu.</p>
                        </motion.div>

                        {/* STICKY BOOKING BAR */}
                        <div className="bg-white/90 backdrop-blur-md rounded-2xl p-4 shadow-2xl border border-white/50 flex items-center justify-between sticky bottom-6 z-50">
                            <div className="pl-2">
                                <p className="text-xl font-black text-gray-900 leading-none">
                                    {formatCurrency(travel?.price)}
                                    <span className="text-xs font-normal text-gray-400 ml-1">/ orang</span>
                                </p>
                                <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-1 italic">Travel Services</p>
                            </div>
                            <Button
                                className="!rounded-xl px-8 py-3.5 !bg-blue-600 hover:!bg-blue-700 text-white font-black transition-all shadow-lg text-sm sm:text-base"
                                disabled={isAdmin || (travel?.is_available === false)}
                                onClick={() => {
                                    if (isAdmin) { alert('Admin tidak bisa booking'); return; }
                                    if (!authToken) {

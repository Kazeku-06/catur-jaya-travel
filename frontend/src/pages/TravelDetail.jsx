    ...
    try {
      ...
      try {
        ...
        const related = (relatedRes.data.data || [])
          .filter(t => t.id !== id)
          .slice(0, 3); // Kita ambil 3 agar sejajar seperti TripDetail
      ...
      <div className="bg-[#F8F9FA] min-h-screen">
        ...
        <div className="container mx-auto px-4 -mt-16 relative z-10 pb-20">
          ...
          <div className="max-w-4xl mx-auto">
              ...
              {travel?.facilities?.length > 0 ? (
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                  {travel.facilities.map((facility, i) => (
                    <div key={i} className="flex items-center p-3 bg-gray-50 rounded-2xl border border-gray-100 group hover:border-blue-200 transition-colors">
                      <div className="flex-shrink-0 bg-blue-100 text-blue-600 p-1.5 rounded-lg mr-3">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                        </svg>
                      </div>
                      <span className="font-bold text-gray-700 text-xs sm:text-sm truncate" title={facility}>
                        {facility}
                      </span>
                    </div>
                  ))}
                    ...
                    <div key={i} className="flex space-x-4 group">
                      ...
                      <div className="pb-6 flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <i className={`fas ${item.time?.toLowerCase().includes('pagi') || item.jam?.toLowerCase().includes('0') ? 'fa-sun text-orange-400' : 'fa-moon text-blue-400'} text-xs`}></i>
                          <span className="text-sm font-black text-blue-600">{item.time || item.jam}</span>
                        </div>
                        <p className="text-sm text-gray-700 font-bold leading-snug uppercase tracking-tight">{item.activity || item.kegiatan}</p>
                      </div>
                    </div>
                  ))
                ) : (
            ...
            {/* STICKY BOOKING BAR (Mirip TripDetail) */}
            <div className="bg-white/80 backdrop-blur-md rounded-2xl p-4 shadow-2xl border border-white/50 flex items-center justify-between sticky bottom-6 z-50">
              <div className="pl-2">
                <p className="text-xl font-black text-gray-900 leading-none">
                  {formatCurrency(travel?.price)} <span className="text-xs font-normal text-gray-400">/ orang</span>
                </p>
          ...
          {/* RELATED SECTION (Full Width Like TripDetail) */}
          {relatedTravels.length > 0 && (
            <div className="mt-20 w-full">
              <div className="flex justify-between items-end mb-8 px-2">
                <div className="text-left">
                  <h2 className="text-2xl font-black text-gray-900">Rekomendasi Travel</h2>
